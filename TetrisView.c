...
void TetrisView_ShowSetting(TetrisView* tetrisView){
	...
	CursorUtil_GotoXY(x, y++);
	printf("早New Level (%d ~ %d)早", MIN_MAP_LEVEL, MAX_MAP_LEVEL);
	CursorUtil_GotoXY(x, y++);
	printf("早:                 早");
	CursorUtil_GotoXY(x, y++);
	printf("曲收收收收收收收收收旭");
	...
	CursorUtil_Hide();
	//SPEED_LEVEL -> MAP_LEVEL 熱薑л*******************************************************************************************************************************
	if (tetrisView->level >= MIN_MAP_LEVEL && tetrisView->level <= MAX_MAP_LEVEL){

	}
	else if (tetrisView->level < MIN_MAP_LEVEL){
		tetrisView->level = MIN_MAP_LEVEL;
	}
	else if (tetrisView->level > MAX_MAP_LEVEL){
		tetrisView->level = MAX_MAP_LEVEL;
	}
	else{
		tetrisView->level = MIN_MAP_LEVEL;
	}
